...
//object definition
class TestJpsiGamma : public edm::one::EDAnalyzer<> {
public:

  //constructor, function is called when new object is created
  explicit TestJpsiGamma(const edm::ParameterSet& conf);
...
private:
  ...
  int nConvPhotons = 0;
  std::vector<int> JpsiG = {22, 443};
  std::vector<int> MuMu = {13, -13};
};


TestJpsiGamma::TestJpsiGamma(const edm::ParameterSet& conf)
...
TestJpsiGamma::~TestJpsiGamma()
{
  cout <<" DTOR" << endl;
}

bool TestJpsiGamma::isSameDecay(const std::vector<int>& dec1, const std::vector<int>& dec2) {
...
void TestJpsiGamma::beginJob()
{
  ...
  cout << "HERE TestJpsiGamma::beginJob()" << endl;
}

void TestJpsiGamma::endJob()
{
  ...
  cout << "HERE TestJpsiGamma::endJob()" << endl;
}


void TestJpsiGamma::analyze(
    const edm::Event& ev, const edm::EventSetup& es)
{
  std::cout << " -------------------------------- HERE TestJpsiGamma::analyze "<< std::endl;
  ...
  for(const auto& genP : genPar)
  {
    if (abs(genP.pdgId()) == 531)
    {
      ...
      if(isSameDecay(daughters, JpsiG))
      {
        for(unsigned int i=0; i < genP.numberOfDaughters(); i++)
        {
          if(abs(genP.daughter(i)->pdgId()) == 443)
          {
            const reco::Candidate* jpsi = genP.daughter(i);
            ...
            if(isSameDecay(jpsiDaughters, MuMu))
            {
              for(unsigned int j=0; j < jpsi->numberOfDaughters(); j++)
              {
                if(abs(jpsi->daughter(j)->pdgId()) == 13)
                {
                  genMuons.push_back(jpsi->daughter(j));
                }
              }
            }
            else
  ...
  for (unsigned int i = 0; i < muonKinematicParticles.size(); i++)
  {
    for (unsigned int j = i+1; j < muonKinematicParticles.size(); j++)
    {
      for (unsigned int k = 0; k < photonKinematicParticles.size(); k++)
      {
        ...
        MultiTrackKinematicConstraint* multiMassConstraint = new TwoTrackMassKinematicConstraint(jpsi_mass);
        // create the combined constraint
        vector<MultiTrackKinematicConstraint*> constraints;
        constraints.push_back(multiPointingConstraint);
        constraints.push_back(multiMassConstraint);
        CombinedKinematicConstraint* combinedConstraint = new CombinedKinematicConstraint(constraints);
        // create the kinematic fitter

        KinematicConstrainedVertexFitter constrainedFitter;
        RefCountedKinematicTree vertexFitTreeGlobal = constrainedFitter.fit(allParticlesGlobal, combinedConstraint);
...
DEFINE_FWK_MODULE(TestJpsiGamma);
